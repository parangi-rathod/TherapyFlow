...
Before starting ANY task, the coordinator and all subagents MUST perform a **repository documentation scan**.

### Step 1 — Scan Documentation

Scan the repository for **all `.md` files** in:
...
### Step 2 — Mandatory Files to Read

The following files MUST be read before implementation:

| File | Purpose |
|---|---|
| `/doc/PRD.md` | Product requirements and feature specs |
| `/doc/TASKS.md` | Master task list with status |
| `/doc/PROGRESS.md` | Timestamped log of completions |
| `/doc/BLOCKERS.md` | Open blockers requiring human input |
| `/doc/CHANGELOG.md` | Code or schema changes |
| `/doc/DECISIONS.md` | Architecture and design decisions |
| `/doc/SCHEMA.md` | Supabase schemas and RLS policies |

### Step 3 — Documentation Summary

Before writing code, the coordinator must produce an **internal summary**:
...
If documentation cannot be read or is missing, **STOP and create the required files before continuing.**

### Step 4 — Documentation Priority Rule
...
Every new Codex session must begin with:

1. Scan repository for `.md` documentation
2. Read `/doc/TASKS.md`
3. Read `/doc/PROGRESS.md`
4. Read `/doc/BLOCKERS.md`
5. Summarize current project state
6. Identify the next incomplete task
7. Continue work from there
...
4. Check documentation constraints
5. Only then modify or create files

Never implement code without first understanding existing architecture.
...
- A bug fix introduces a new pattern
...
| PROGRESS.md | Every work session |
